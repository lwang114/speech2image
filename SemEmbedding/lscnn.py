...
def load_logspec_from_wavfile(filename, mfcc):
    ...
    logspec = mfcc.sig2logspec(caption_info[1])
...
def set_nframes_to_fixed_size(logspec, target_framecount):
    ...
    margin = round((nframes-target_framecount)/2)
    logspec_new = np.zeros((logspec.shape[0], Leq))
    if margin < 0:
...
'''
    def __init__(self, nchan_scnn=[40,64,512,1024], width_scnn=[5,25,25], strid_scnn=[2,2,256]):
        '''lscnn = LSCNN(nchan_scnn, width_scnn, strid_scnn, use_case)
        ...
        self.pmtrs += self.vggembed(8192, nembed)
        ...
        self.init = tf.initialize_all_variables() # randomly initialize
        self.sess.run(init)
    ...
    def load_data(self, numtoks, wav2capt_filename='../data/flickr_audio/wav2capt.txt', dir_sp='../data/flickr_audio/wavs/', dir_penult='../data/vgg_flickr8k_nnet_penults/'):
        '''lscnn.load_data(num_toks, wav2capt_filename, speech_dir, vgg16_dir)
        ...
        with open(wav2capt_filename, 'r') as f:
            for i in range(numtoks):
                cur_info_parts = f.readline().rstrip().split()
    ...
    def scnn(self, nchan_scnn, width_scnn, strid_scnn):
        ...
        X_in = tf.placeholder(tf.float32, shape=[None,1,self.target_framecount,self.nfilts])
        
        # Subtract mean spectrogram estimated over the entire training set
        self.X_mean = tf.reduce_mean(X_in)
        self.X_zm = X_in - self.X_mean
        ...
        self.a_node = [ X_in ]
        for layer in len(N):
            self.w[layer] = weight_variable([1, width_scnn[layer], nchan_scnn[layer], nchan_scnn[layer+1]])
            self.b[layer] = bias_variable([nchan_scnn[layer+1]])
            self.a_conv[layer] = tf.nn.conv2d(self.a_node[layer], self.w[layer], strides=[1,1,1,1], padding='SAME') + self.b[layer]
            if layer < len(N):
                maxpool_padding = 'SAME'
            else:
                maxpool_padding = 'VALID'
            self.a_pool[layer] = tf.nn.max_pool(self.a_conv[layer], ksize=[1, 1, 2*strid_scnn[layer], 1], strides=[1, 1, strid_scnn[layer], 1], padding=maxpool_padding)
            self.a_node[layer+1] = tf.nn.relu(self.a_pool[layer])
    ...
    def train(self, training_toks):
        ...
        for t in range(niter):
            ...
            for i in range(nbatch):
                ...
                self.sess.run(self.train_step, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                if i % 10 == 0:
                    similarity = self.sess.run(self.s, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                    cur_cost = self.sess.run(self.cost, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                    ...
                    cur_z_sp = sess.run(self.Z_embed_sp, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                    cur_z_im = sess.run(self.Z_embed_vgg, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                    cur_s = sess.run(self.s, feed_dict={X_in:X_batch_4d, Z_penult_vgg:Z_batch})
                    print('Training similarity score is:\n', cur_s)
                    print('Iteration', t, 'at batch', i)
                    print('Training accuracy is: ', tr_accuracy[t])

    ##########################################################
    def scnn_test(self, testing_toks):
        '''lscnn.test( testing_toks )
        ...
        similarity = self.sess.run(self.s, feed_dict={X_in:X_dev_4d, Z_penult_vgg:Z_dev})
        cur_cost = self.sess.run(self.cost, feed_dict={X_in:X_dev_4d, Z_penult_vgg:Z_dev})
        # Find the indices of the images with the top 10 similarity score
        ntop = 10
