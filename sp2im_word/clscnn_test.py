...
def clscnn_test(data_file, model_file, nclass, hinge_loss=False, sentence=False):
  # Load the data from h5 file
  h5_feat = h5py.File(data_file)
  data_dir = '/'.join(data_file.split('/')[:-1]) + '/'
  grp_feat_tx = h5_feat['val']
  ...
  a_feat_tx = np.transpose(a_feat_tx[:, np.newaxis, :, :], [0, 1, 3, 2]) 
  v_feat_tx = np.squeeze(v_feat_tx, axis=1)
 
  if sentence:
    a_feat_tx_sample, v_feat_tx_sample, y_tx_sample = select_retrieval_database(a_feat_tx, v_feat_tx, y_tx, data_dir + 'ind2sent_val_cleanup.txt', data_dir + 'im2sent_val.json')
  ...
  if sentence:
    print(recall_op_sentence(similarity, y_tx_sample, 1, 'recall_at_1_sent.txt'))
    print(recall_op_sentence(similarity, y_tx_sample, 5, 'recall_at_5_sent.txt'))
    print(recall_op_sentence(similarity, y_tx_sample, 10, 'recall_at_10_sent.txt'))
    print(recall_op_sentence(similarity, y_tx_sample, 1, 'recall_at_1_sent.txt', binary_error=True))
    print(recall_op_sentence(similarity, y_tx_sample, 5, 'recall_at_5_sent.txt', binary_error=True))
    print(recall_op_sentence(similarity, y_tx_sample, 10, 'recall_at_10_sent.txt', binary_error=True))
...
if __name__ == '__main__':
  ...
  clscnn_test('/home/lwang114/spring2018/sp2im_word/data/flickr_sentence/flickr_sent_fbank_penult_order.h5', '/home/lwang114/spring2018/sp2im_word/sp2im_wrd_model_tflearn-18-08-15-10', 70, hinge_loss=True, sentence=True)
